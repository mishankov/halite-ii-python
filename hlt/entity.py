...
class Ship(Entity):
    ...
    def navigate_x(self, target, game_map, speed):
        distance = self.calculate_distance_between(target)
        angle = self.calculate_angle_between(target)
        ...
        if not game_map.obstacles_between(new_target):
            return self.thrust(speed, angle)
        else:
            new_angel = angel + 45
            new_target = Position(
                self.x + math.cos(math.radians(new_angel)), 
                self.y + math.sin(math.radians(new_angel)))
            if not game_map.obstacles_between(new_target):
                return self.thrust(new_target, new_angel)
            else:
                new_angel = angel - 45
                new_target = Position(
                    self.x + math.cos(math.radians(new_angel)), 
